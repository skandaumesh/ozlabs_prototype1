  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        // Once logged in, the portal layout will handle fetching the client data
        router.push('/portal');
        router.refresh();
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="max-w-sm w-full space-y-10">
        <div className="text-center">
          <h1 className="text-4xl font-extrabold tracking-tighter">CLIENT PORTAL</h1>
          <p className="text-muted text-sm font-medium mt-3">Sign in to your dashboard</p>
        </div>
